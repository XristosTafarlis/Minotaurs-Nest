...
public class EnemyAI : MonoBehaviour{
	...
	int size;			//Refference to maze size
	...
	public static bool minotaruIsDead;
	...
	private void Awake(){
		...
		size = mazeRenderer.GetComponent<MazeRenderer>().size;		//Maze size
		width = mazeRenderer.GetComponent<MazeRenderer>().width;		//Wall width
	}
	...
	private void SearchWalkPoint(){
		...
		float randomX = Random.Range( -(size/2), size/2 ) * width;
		float randomZ = Random.Range( -(size/2), size/2 ) * width;
